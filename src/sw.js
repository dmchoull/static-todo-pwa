/* eslint-disable no-console,no-restricted-globals,compat/compat */
...
self.addEventListener('activate', (event) => {
  ...
  // Clean the caches
  event.waitUntil(global.caches.keys().then(cacheNames => Promise.all(cacheNames.map((cacheName) => {
    ...
    return fetch(request)
      .then((responseNetwork) => {
        if (!responseNetwork || !responseNetwork.ok) {
          if (DEBUG) {
            console.log(`[SW] URL [${requestUrl.toString()}] wrong responseNetwork: ${
